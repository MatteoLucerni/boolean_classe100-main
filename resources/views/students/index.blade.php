...
@section('main')
    <div class="container py-5">
        <h1 class="text-center text-white my-5">STUDENTI CLASSE 100#</h1>
                ...
                <li class="card my-3 p-4 shadow">
                    <div class="row">
                        ...
                        <div class="col-6 right-side d-flex align-items-center ">
                            <a class="btn btn-info text-white me-3 shadow " href="{{ route('students.show', $student) }}">Dettagli
                                studente</a>
                            <a class="btn btn-success shadow" href="{{ route('students.create') }}">Aggiungi studente</a>
                            <a class="btn btn-warning mx-3 shadow" href="{{ route('students.edit', $student) }}">Modifica
                                studente</a>
                            <form data-name="{{ $student->first_name }} {{ $student->last_name }}" class="delete-form"
                                action="{{ route('students.destroy', $student) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger shadow">Elimina studente</button>
                            </form>
                        </div>
                    </div>
                </li>
